...
const creaetValidator = <T = unknown>(
  ...
  if (option?.defaultValue) v = v.default(option.defaultValue);
  return v;
};

export const string = (option?: ValidatorOption<string>) =>
  creaetValidator<string>(Joi.string(), option);

export const number = (option?: ValidatorOption<number>) =>
  creaetValidator<number>(Joi.number(), option);

export const boolean = (option?: ValidatorOption<boolean>) =>
  creaetValidator<boolean>(Joi.boolean(), option);

export const array = <T = unknown>(
  item: Joi.Schema<T>,
  options?: ValidatorOption<T>
) => {
  let v = creaetValidator<T>(
